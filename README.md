...
The output payload (`GeneratedOutputPayload`) is shape-checked server-side via the TypeScript contract in [lib/outputTypes.ts](lib/outputTypes.ts) and only emitted after the structured pipeline phases complete.
...
The wizard supports five sector tracks — **Government**, **Business**, **Education**, **Healthcare**, and **Other** (non-profit, research, etc.) — and tailors its problem prompts to each. It shows a privacy disclaimer above any data input, and the DB connector reinforces that credentials are used once and never stored.
...
lib/                Server-side logic (pipelinePhases, generalPipeline,
                    analyzePipeline, buildOutputPayload, benchmarks,
...
The product should leave a viewer convinced that OpsAdvisor can take a real operational workflow problem — whether it's a sales pipeline, a hospital intake queue, a student support process, a grant cycle, or a permitting workflow — identify the biggest source of leakage, recommend a credible first fix, and hand off controls the team can actually use.
